...
async function readPackages(sessionRef: FirebaseFirestore.DocumentReference) {
  const packageSnapshot = await sessionRef.collection("packages").get();
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!canAccessDevice(user.profile, registered)) {
      return NextResponse.json(
        { success: false, error: "Tidak memiliki akses ke device ini" },
        { status: 403 },
      );
    }
    ...
    if (runtime) {
      ...
      const sessionRef = adminDb.collection("sessions").doc(runtime.activeSessionId);
      const packages = includePackages ? await readPackages(sessionRef) : [];
    ...
    /* Legacy fallback sebelum device_runtime dimigrasikan. */
    ...
    const packages = includePackages ? await readPackages(sessionDoc.ref) : [];
